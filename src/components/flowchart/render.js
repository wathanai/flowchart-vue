...
function render(g, node, isSelected) {
  ...
  if (node.type !== 'start' && node.type !== 'end') {
    ...
    body
      .attr('x', node.x)
      .borderColorattr('y', node.y)
      .borderColorclassed(node.type, true)
      .borderColorattr('rx', 30)
  ...
  // body text
  let text =
    node.type === 'start' ? 'Start' : node.type === 'end' ? 'End' : 'Unknown'
  let bodyTextY
  if (node.type !== 'start' && node.type !== 'end') {
